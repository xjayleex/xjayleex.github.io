...
- T2 인스턴스에서 기본 사용률에 필요한 것보다 더 적은 CPU 리소스를 사용하는 경우(e.g. Idle Time) 사용하지 않은 CPU 크레딧은 CPU 크레딧 밸런스에 누적된다.
- CPU 크레딧 밸런스 한도는 시간당 받는 크레딧 * 24 이다.
- CPU 크레딧 밸런스는 24시간 동안 사용할 수 있고, 획득 후 24시간이 지나면 만료된다.
- 인스턴스 정지 및 시작 사이에는 CPU 크레딧이 지속되지 않는다. 인스턴스 시작 후 인스턴스는 초기 CPU 크레딧을 다시 받는다.
- CPU 크레딧 밸런스가 완전히 소진되면 인스턴스는 baseline 성능으로 수행된다.

##### **C4 Instances (Compute Intensive)**
---
- 컴퓨팅 최적화 인스턴스는 고성능 프로세서의 이점을 활용하는 Compute Intensive 워크로드에 적합
- 다음과 같은 경우에 적합
- 배치 작업, 미디어 트랜스코딩, 고성능 웹서버, HPC, 과학 모델링, 게임 서버, 광고 서비스 엔진, 기계학습
- 기본적으로 EBS 최적화되어 있으며, 향상된 네트워킹을 사용 할 수 있음.
- 배치 그룹에서 클러스터링 가능
- 64-bit HVM AMI가 필요
- VPC로만 시작할 수 있으며 EC2-Classic 플랫폼에서는 지원되지 않음.

##### **G2 Instances (Graphic Intensive)**
---
- GPU 인스턴스는 높은 병렬 처리 기능을 제공
- CUDA 또는 OpenCL 병렬 컴퓨팅 프레임워크를 활용하여 과학, 엔지니어링 및 렌더링 애플리케이션을 가속화하는 데 적합.
- 게임스트리밍, 3D 애플리케이션 스트리밍 및 그래픽 애플리케이션에 적합
- HVM AMI 필요.
- NVIDIA 드라이버가 설치되어 있지 않으면 GPU에 액세스 할 수 없음
- 배치 그룹에서 클러스터링 가능

##### **I2 Instances (I/O Intensice)**
---
- I2 인스턴스는 low-latency, Random I/O를 요구하는 애플리케이션에 최적화.
- NoSQL DB, Clustering된 DB, OLTP 시스템과 같은 애플리케이션에 적합
- 기본 데이터 스토리지는 SSD 기반 Instnace 스토리지
- 향상된 네트워킹 사용 가능
- 배치 그룹에서 클러스터링 가능
- EBS 최적화를 통해 Amazon EBS I/O 전용 용량을 추가로 확보 가능
- HVM AMI 필요( PV, HVM AMI 모두 지원하는 전세대 인스턴스 HI1과 동일)

##### **D2 Instances(Density Instensive)**
---
- D2 인스턴스는 스토리지 밀도가 굉장히 높은 워크로드를 위해 설계되었으며 로컬 스토리지의 매우 큰 데이터 세트에 대한 높은 Sequential I/O 액세스 지원
- 대규모 병렬 처리(MPP) 데이터 웨어하우스, 맵리듀스 및 하둡 분산 컴퓨팅, 로그 or 데이터 처리 애플리케이션에 적합
- D2 인스턴스의 기본 데이터 스토리지는 HDD 기반 Instance  스토리지
- 기본적으로 EBS 최적화되어 있으며, 향상된 네트워킹을 사용 할 수 있음.
- 배치 그룹에서 클러스터링 가능
- 64-bit HVM AMI 필요
- HS1은 동일한 이전 세대 인스턴스로, EBS 및 Intance Store AMI 모두 지, PV 및 HVM AMI 모두 지
